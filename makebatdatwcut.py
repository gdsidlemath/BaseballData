...
def getbatterstats(bInds,bpt,typebs,ID,bh,stan):
    ...
    outmat = np.column_stack((temp1,temp2,temp3,nums))
...
def makebatterdata(Data):
    
    inds2016 = np.where(Data.gameID > 201606000000)[0]
    
    batterVec = np.asarray(list(set(Data.batter[inds2016])))
    
    print len(batterVec)
    
    bNums = np.zeros([len(set(Data.batter[inds2016])),1])
    
    """for i in range(0,len(pitcherVec)):
        bNums[i] = len(np.where(Data.batter == batterVec[i])[0])"""
    
    allbat = np.zeros([len(batterVec),27])
    
    mainstart = time.clock()
    
    for k in range(0,len(batterVec)):
        ...
        bInds = np.where(Data.batter == b)[0]
        
        bh = Data.bheight[bInds[0]]
        
        stan = fixstand(Data.stance[bInds[0]])
        
        batmat = getbatterstats(bInds,pitchscore(Data.outpitch[bInds]),Data.res_typebs,b,bh,stan)[0]
        
        #print batmat
        
        #print 'Original length ' + str(len(bInds))
        
        #print np.sum(batmat[21:25])
        ...
        YearCounts[8:14,k] = getcount(outpitchgood).T[:,0]
        YearCounts[14,k] = len(balls)
        YearCounts[15,k] = len(strikes)
        YearCounts[16,k] = len(inplay)
        YearCounts[17,k] = p
        ...
        str2 = '%i ' * 8 + '%5.6f ' * 6 + '%i %i %i %i'

        np.savetxt('PitcherData/' + str(p) + 'Data2014p.txt',pDataMatrix,str1)
        np.savetxt('PitcherData/' + str(p) + 'Outputs2014p.txt',pOutMatrix,'%i %2.2f %i %i')
        #np.savetxt('PitcherData/YearCountStuff.txt',YearCounts.T,str2)"""
        
        str1 = '%5.6f ' * 21 + '%i %i %i %i %i %i'
        
        np.savetxt('PitDataUpdate/BatterswCutterUpdate.txt',allbat,str1)
